...
def MakeBayerImage(Width, Height):
	...
	RGRow = itertools.cycle("RG")
	GBRow = itertools.cycle("GB")
	...
	return [[GetRandomColorShade(BayerImageList[i][j]) for j in range(Width)] for i in range(Height)]
...
def Intro():
	DisplayIntro("Bayer's Filter & Demosaicing Algorithm (Randomized Colors)", TextColor1, 50, -100)
...
pygame.display.set_caption("Bayer's Filter & Demosaicing Algorithm (Randomized Colors)")
screen.fill(BGColor)
